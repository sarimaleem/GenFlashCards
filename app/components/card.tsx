...
export default function Card({ id, word, definition }: { id: number, word: string, definition: string }) {
  return (
    <div className='flex py-2 my-2 mx-4 px-[2em] border border-dotted border-black rounded'>
      ...
      <button className='border border-black border-dotted rounded h-[2.5rem] aspect-square px-2 text-xs mx-2 ml-auto' onClick={() => { cardDeleteHandler(id) }}>
        delete
      </button>
      <EditCard id={id} word={word} definition={definition}></EditCard>
      <button className='border border-black border-dotted rounded h-[2.5rem] aspect-square px-2 text-xs mx-2' onClick={() => { cardClickHandler(id) }}>
        review
      </button>
    </div>
  )
